...
export default class APIClient {
  ...
  constructor() {
    this.init = {
      headers: new Headers({
        Accept: 'application/json',
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${STATIC_TOKEN}` // Use the static token here
      })
    };
  ...
  //   });
  // }
  async fetch(path: string, params?: any, method: string = 'GET', options: { clean?: boolean } = { clean: true }): Promise<Response> {
    const init = this.getInit(method, options.clean && params ? clean(params) : params);
    ...
    return fetch(edp + path, init).then(response => {
      if (response.ok) {
        ...
        return Promise.reject({ message: `! ${method} error on ${path}; ${response.status}`, response });
      }
    }).catch(error => {
      return Promise.reject({ message: `! ${method} error on ${path}; ${error.message}` });
    });
  }
